...
function compare(a, b) {
    var r = 0;
    const createDateA = parseISO(a.created_at).getTime()
    const createDateB = parseISO(b.created_at).getTime()
...
export async function getPostsByCategoryId(id) {
    ...
    const categoryPosts = posts.filter(post => post.categories != null).filter(
        post => post.categories.filter(category => category.id == id).length >= 1)
